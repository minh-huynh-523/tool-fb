...
export function PromptTemplateForm({ initialBody, updatedAt }: { initialBody: string; updatedAt: string }) {
  const router = useRouter();
  const [body, setBody] = useState(initialBody);
  const [saving, setSaving] = useState(false);

  const dirty = body !== initialBody;
  const missing = REQUIRED_HEADINGS.filter((h) => !body.toUpperCase().includes(h));
  ...
  async function save() {
    ...
    try {
      const { res, data } = await fetchJson("/api/prompt-templates", {
        ...
        body: JSON.stringify({ kind: "main", body }),
      });
