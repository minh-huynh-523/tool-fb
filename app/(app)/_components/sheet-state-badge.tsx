...
// Nhãn "page này đã copy sang Sheet chưa". Dùng ở cả trang danh sách lẫn trang chi tiết.
// 'có bài mới' = đã copy nhưng worker cào thêm bài sau đó → còn bài chưa copy.
const STYLE: Record<SheetState, string> = {
  "đã copy": "bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-300",
  "có bài mới": "bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300",
...
export function SheetStateBadge({ state, copiedAt }: { state: SheetState; copiedAt: string | null }) {
  return (
    ...
    >
      {state === "có bài mới" ? "có bài mới sau lần copy" : state}
    </span>
  );
}
