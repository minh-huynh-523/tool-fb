// Implementation của client Gemini — tách khỏi lib/gemini.ts để dùng được cả từ script chạy bằng
// tsx (scripts/scrape-worker.ts, worker Node thuần) lẫn từ Next.js server code. lib/gemini.ts vẫn
// là điểm import CHO NEXT.JS (giữ marker 'server-only' chặn lỡ tay import vào Client Component) —
// KHÔNG import file này thẳng từ code chạy trong trình duyệt/Client Component.
//
// Client Gemini tối giản — gọi REST bằng fetch, KHÔNG thêm SDK (đúng kiểu các client khác trong
// repo). Chỉ dùng generateContent, không stream: output được lưu thẳng vào DB nên không cần hiển
// thị dần.
