// Port của lib/post-image-backup.ts cho Deno — logic Y HỆT bản Node (Buffer -> Uint8Array).
import type { SupabaseClient } from "npm:@supabase/supabase-js@2";
import { uploadPublicImage } from "./storage.ts";

const UA =
  "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0 Safari/537.36";
const BUCKET = "post-media";
// Edge Function có ngân sách thời gian riêng (không chung 60s với sync-pages/processDueComments
// như ở Vercel) nhưng vẫn giữ 1 trần hợp lý mỗi lượt — phần dư dồn qua lượt cron sau.
...
export async function backupPostImages(
  ...
  for (const p of due) {
    ...
    try {
      const res = await fetch(p.media_url, { headers: { "User-Agent": UA } });
