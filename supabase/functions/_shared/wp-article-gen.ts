...
export async function generateWpArticleFromFb(db: SupabaseClient, postDbId: string): Promise<ScrapedArticleLike> {
  ...
  return {
    ...
    contentHtml,
    description: "",
    // CHỈ dùng ảnh đã backup vào Supabase Storage — KHÔNG fallback về post.media_url (link CDN
    // của FB, hay hết hạn/chặn hotlink). Xem lib/wp-article-gen.ts bản Next.js để biết lý do đầy đủ.
